...
def fetch_url(url: str):
    ...
    try:
        for _ in range(MAX_REDIRECTS + 1):
            ...
            resp = session.get(current, timeout=REQUEST_TIMEOUT, allow_redirects=False, stream=True)
...
def format_ssl(cert) -> str:
    ...
    def _name(entries) -> str:
        try:
            ...
            return str(entries)
    sans = [f"{kind}: {value}" for kind, value in cert.get("subjectAltName") or []]
    lines = [
...
def get_ip_info(ip: str) -> str:
    lines = [f"IP: {ip}"]
    try:
        resp = requests.get(f"https://ipinfo.io/{ip}/json", timeout=8, headers={"User-Agent": USER_AGENT})
        if resp.status_code == 200:
            data = resp.json()
            org = data.get("org", "N/A")
            location = f"{data.get('city', 'N/A')}, {data.get('region', 'N/A')}, {data.get('country', 'N/A')}"
            hostname = data.get("hostname", "N/A")
            lines += [f"      Org      : {org}", f"      Location : {location}", f"      Hostname : {hostname}"]
        else:
...
def detect_waf(headers: dict, dns_records: dict):
    wafs = []
    header_text = " ".join(str(v).lower() for v in headers.values())
    keys = {k.lower(): v for k, v in headers.items()}
    if "cf-ray" in keys or "cloudflare" in header_text:
    ...
    if any("i2.checkpoint.com" in str(c).lower() for c in cname_records):
        ...
        cloudguard = True
    return ("\n".join(wafs) if wafs else "No WAF detected"), cloudguard
...
def _empty_report(url: str, domain: str, error=None) -> dict:
    return {
        "url": url, "domain": domain, "error": error, "cloudguard": False,
        "status": "", "performance": "", "headers": "", "dns_full": "",
        "dns_records": {}, "ip_info": "", "cdn": "", "load_balancer": "",
        "waf": "", "third_party": "", "whois": "", "ssl": "", "tech": "",
        "smart": "", "security": "", "summary": "",
...
def scan(raw_url: str) -> dict:
    ...
    report["ip_info"] = "\n\n".join(get_ip_info(ip) for ip in a_records) if a_records else "No A records"
    ...
    summary_bits = [
        ...
        "CloudGuard WAF auto-detected if i2.checkpoint.com CNAME is present" if cloudguard else "No Check Point CloudGuard CNAME (i2.checkpoint.com) observed",
...
def save_beautiful_report(domain: str, data: dict) -> str:
    ...
    def safe_str(value):
        ...
        return str(value)
    html = "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Website Detective</title></head><body><pre>" + escape(safe_str(data.get("performance"))) + "</pre></body></html>"
...
if __name__ == "__main__":
    ...
    if result.get("error"):
        ...
        print("Investigating:", result.get("url"))
